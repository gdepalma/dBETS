...
source('R/ModelFunctionsOne.R')
source('R/ERB/ERBFunctions.R')
source('R/ERB/ERBC.R')
source('R/ERB/ERBOneBrkpt.R')
source('R/Spline/plotSpline.r')
source('R/Logistic/logisticFunctions.R')
source('R/Logistic/runLogistic.r')
source('R/Logistic/runLogisticOne.r')
source('R/Logistic/plotLogistic.r')
source('R/Spline/runSpline.r')
source('R/Spline/runSplineOne.r')
source('R/Spline/plotSpline.r')
...
shinyServer(function(input, output, session) {
      ...
      return(isolate({
        if(input$panel==4){
          if(input$oneBrkpt==FALSE){
            ...
            m2=as.numeric(input$m23)
            convert=FALSE
            if(input$miclogERB3=='1') convert=TRUE
  ...
  output$plotLogisticDensity <- renderPlot({
    if(input$actionLogPlot!=0){
      return(isolate({
        if(input$panel2==2){
          logConvert=FALSE
          if(input$miclogM1=='1') logConvert=TRUE
          if(input$oneBrkpt==FALSE){
            a1=data.frame(grid=xgrid,weights=medianDensityLog)
            fit=plotUnderlyingDistibution(a1,logConvert,MICBrkptL+1,MICBrkptU)
            plot(fit)
          }
          if(input$oneBrkpt==TRUE){
            a1=data.frame(grid=xgrid,weights=medianDensityLog)
            fit=plotUnderlyingDistibutionOne(a1,logConvert,MICBrkpt+1)
            plot(fit)
          }
        }
      }))
  ...
  output$plotSplineDensity <- renderPlot({
    if(input$actionSplinePlot!=0){
      return(isolate({
        if(input$panel2==3){
          logConvert=FALSE
          if(input$miclogM2=='1') logConvert=TRUE
          if(input$oneBrkpt==FALSE){
            a1=data.frame(grid=xgrid,weights=medianDensitySpline)
            fit=plotUnderlyingDistibution(a1,logConvert,MICBrkptL+1,MICBrkptU)
            plot(fit)
          }
          if(input$oneBrkpt==TRUE){
    ...
    if(input$actionProbDIA!=0){
      return(isolate({
        if(input$panel2==5){
          ...
          if(input$oneBrkpt==FALSE){
            if(input$modelSelect=='spline'){
              a1=data.frame(grid=xgrid,weights=dataSpline$dens/sum(dataSpline$dens))
              plotDist <<- plotUnderlyingDistibution(a1,convert,MICBrkptL,MICBrkptU)
            }else{
              a1=data.frame(grid=xgrid,weights=dataLog$dens/sum(dataLog$dens))
              plotDist <<- plotUnderlyingDistibution(a1,convert,MICBrkptL,MICBrkptU)
            }
          }else{
            if(input$modelSelect=='spline'){
              a1=data.frame(grid=xgrid,weights=dataSpline$dens/sum(dataSpline$dens))
              plotDist <<- plotUnderlyingDistibutionOne(a1,convert,MICBrkpt)
            }else{
              a1=data.frame(grid=xgrid,weights=dataLog$dens/sum(dataLog$dens))
              plotDist <<- plotUnderlyingDistibutionOne(a1,convert,MICBrkpt)
            }          
