...
plotLogistic=function(MIC,DIA,xcens,ycens,M1,M2,D1Log,D2Log,medianDensity,medFit,
                      lowerFit,upperFit,flipGraph,logConvert){
    
  log_fit_data=data.frame(grid=xgrid,Median=medFit,Lower=lowerFit,Upper=upperFit)

  # write_csv(a1,'logistic_fit.csv')
  # save.image(file='yoursession.RData')
  # load('yoursession.RData')
  # log_fit_data=read_csv('logistic_fit.csv')
  ...
  if(flipGraph=='Yes' && logConvert==TRUE){
    fit = ggplot(a1,aes(xobs,yobs))+geom_text(aes(label=freq),size=5)+
      geom_ribbon(data=log_fit_data,aes(x=xgrid,ymin=Lower,ymax=Upper),
                  fill="skyblue2",alpha=.7, inherit.aes = FALSE)+
      geom_line(data=log_fit_data,aes(x=grid,y=Median),color='blue',inherit.aes = FALSE)+
      geom_vline(xintercept=M1,lty=2,alpha=.5)+
      geom_vline(xintercept=M2,lty=2,alpha=.5)+
      geom_hline(yintercept=D1Log+.5,lty=2,alpha=.5)+
      geom_hline(yintercept=D2Log-.5,lty=2,alpha=.5)+
  ...
  if(flipGraph=='Yes' && logConvert==FALSE){
    ...
    M2=2^M2
    a1$xobs=2^a1$xobs
    MICTemp=c(min(MIC)-1,min(MIC):max(MIC),max(MIC)+1)
    MICTemp=2^MICTemp
    x=2^(min(MIC):max(MIC))
    log_fit_data$grid=2^log_fit_data$grid
    fit=ggplot(a1,aes(xobs,yobs))+geom_text(aes(label=freq),size=5)+
      geom_ribbon(data=log_fit_data,aes(x=grid,ymin=Lower,ymax=Upper),
                  fill="skyblue2",alpha=.7, inherit.aes = FALSE)+
      geom_line(data=log_fit_data,aes(x=grid,y=Median),color='blue',inherit.aes = FALSE)+
      geom_vline(xintercept=M1,lty=2,alpha=.5)+
      geom_vline(xintercept=M2,lty=2,alpha=.5)+
  ...
  if(flipGraph=='No' &&  logConvert==TRUE){
    fit=ggplot(a1,aes(yobs,xobs))+geom_text(aes(label=freq),size=5)+
      geom_ribbon(data=log_fit_data,aes(y=grid,xmin=Lower,xmax=Upper),
                  fill="skyblue2",alpha=.7, inherit.aes = FALSE)+
      geom_line(data=log_fit_data,aes(y=grid,x=Median),color='blue',inherit.aes = FALSE)+
      geom_vline(xintercept=D1Log+.5,lty=2,alpha=.5)+
      geom_vline(xintercept=D2Log-.5,lty=2,alpha=.5)+
...
plotLogisticOne=function(MIC,DIA,xcens,ycens,MICBrkpt,DIABrkpt,medianDensity,medFit,
                      lowerFit,upperFit,flipGraph,logConvert){
