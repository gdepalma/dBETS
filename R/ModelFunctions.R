...
compareFitsPlot=function(dataSpline,dataLog,MIC,DIA,xcens,ycens,M1,M2,flipGraph,logConvert){
  ...
  splineData=data.frame(grid=xgrid,fit=dataSpline$fit,dens=dataSpline$dens/sum(dataLog$dens),group="Spline")
  ...
  if(flipGraph=='Yes' && logConvert==FALSE){
    ...
    MICBrkptL=2^(MICBrkptL+.5)
    MICBrkptU=2^(MICBrkptU-.5)
    a1$xobs=2^a1$xobs
    MIC2=MIC
  ...
  fit <- gridExtra::arrangeGrob(pltRel, plotDens,ncol=1, heights=c(7,4))
