...
double calcLossTrueMin(double DIABrkpt,double *gridx,double *weights,double *f,double MTest,double MTrue,double e,double t,int lgrid){
  ...
  for(i=0; i<lgrid; i++){
    temp=weights[i]*pow(min(0,pdia(f[i],gridx[i],DIABrkpt,MTrue,t)-pmicTrue(gridx[i],MTest,MTrue,e)),2);
    sum+=temp;
  }
  return(sum);
}
...
void findDIATrueOne(double *gridx,double *weights,double *fit,double *MTest, double *MTrue,
  ...
  for(x=*minDIA; x<=*maxDIA; x++){
    test=calcLossTrueMin(x,gridx,weights,fit,*MTest,*MTrue,*xsig,*ysig,*lgrid);
