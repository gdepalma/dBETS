...
fluidPage(theme=shinythemes::shinytheme("cosmo"),
  ...
  sidebarPanel(
    ...
    conditionalPanel(condition="input.Page==1",
      ...
      conditionalPanel(condition = "input.downloadData!=0",
        br(),div("Graph Options: "),br(),
        fluidRow(
          column(6,selectInput("FlipS", "X-Axis:",list('MIC'='Yes','DIA'='No'),selected='No')),
          column(6,selectInput(inputId='miclogS',label='Log2 Labels',choices=list('Yes'=1,'No'=0),selected=1))
        ),
      conditionalPanel(condition="input.downloadData!=0",
      ...
      wellPanel(
        ...
        conditionalPanel(condition="input.panel==2",
          ...
          conditionalPanel(condition="input.actionERB!=0",
             ...
             actionButton('plotERB1',strong('Plot ERB Graph')))),
        ...
        conditionalPanel(condition="input.panel==3",
          conditionalPanel(condition = "input.oneBrkpt == false",
            ...
            div("Error Percentages outside one of Intermediate Range"),
            fluidRow(
              column(4,selectInput("VM22", "VM:",choices =seq(0,30,by=.5),selected=2)),
              column(4,selectInput("M22", "Major:",choices =seq(0,30,by=.5),selected=2)),
              column(4,selectInput("m22", "Minor:",choices =seq(0,60,by=.5),selected=5))
            )),
            conditionalPanel(condition = "input.oneBrkpt == true",
             fluidRow(
        ...
        conditionalPanel(condition="input.panel==4",
          conditionalPanel(condition = "input.oneBrkpt == false",
            ...
            div("Error Percentages outside one of Intermediate Range"),
            fluidRow(
              column(4,selectInput("VM23", "VM:",choices =seq(0,30,by=.5),selected=2)),
              column(4,selectInput("M23", "Major:",choices =seq(0,30,by=.5),selected=2)),
              column(4,selectInput("m23", "Minor:",choices =seq(0,60,by=.5),selected=5))
            )),
          conditionalPanel(condition = "input.oneBrkpt == true",
          ...
          conditionalPanel(condition="input.actionERBSelected!=0",
            br(),div("Graph Options: "),br(),
            fluidRow(
              column(5,selectInput("FlipERB3", "X-Axis:",list('MIC'='Yes','DIA'='No'),selected='No')),
              column(5,selectInput('miclogERB3', 'Log2 Labels',list('Yes'=1,'No'=0),selected=1))
            ),br(),
            actionButton('plotERBSelected',strong('Plot ERB Selected Graph'))))             
    )),
       ...
       wellPanel(
         ...
         #### first panel
          conditionalPanel(condition="input.panel2==2",
            ...
            selectInput("lossType1", "Loss Function:",list('Squared'=1),selected=1),
         ...
         ### Second panel
          conditionalPanel(condition="input.panel2==3",
            ...
            selectInput("lossType2", "Loss Function:",list('Squared'=1),selected=1),
         ...
         conditionalPanel(condition="input.panel2==4",
            fluidRow(
              column(5,selectInput("FlipMod3", "X-Axis:",list('MIC'='Yes','DIA'='No'),selected='No')),
              column(5,selectInput('miclogM3', 'Log2 Labels',list('Yes'=1,'No'=0),selected=1))
            ),
            br(),
         ...
         conditionalPanel(condition="input.panel2==5",
          selectInput("modelSelect", "Model:",list('Spline'='spline','Logistic'='logistic'),selected='spline'),br(),
          conditionalPanel(condition = "input.oneBrkpt == false",
             fluidRow(
    ...
    conditionalPanel(condition="input.Page==1",
                     list(
                       conditionalPanel("input.downloadData==0",
                                        ...
                                        br(),
                                        div(HTML("Software package created by Glen DePalma Ph.D. (<a href=https://www.linkedin.com/in/glen-depalma-01625698/>https://www.linkedin.com/in/glen-depalma-01625698/</a>) 
         and Bruce A. Craig Ph.D. (<a href=https://www.stat.purdue.edu/people/faculty/bacraig.html>https://www.stat.purdue.edu/people/faculty/bacraig.html</a>); with advice from John Turnidge Ph.D.")),br(),
  ...
  The output will appear in the main section on the right once the procedure completes.  Data can be uploaded via the local hard drive or from a URL. <b>For information on how to format data for use with dBETS 
    <a href=https://dbets.shinyapps.io/dBETS/Formatting_Data.pdf>click here.</a></b>")),br(),
                                        div(HTML("Please contact Glen DePalma, <a href=mailto:glen.depalma@gmail.com>glen.depalma@gmail.com</a>, to report problems or suggestions.  The code is located 
         on GitHub <a href=https://github.com/gdepalma/dBETS>https://github.com/gdepalma/dBETS</a> and is under GPL-3 license (feel free to use, modify, and distribute; but source code must be made available).")),br(),
                                        div(HTML("<hr>")),
  ...
  <i>Veterinary Microbiology.</i> 2018. 214 [56-64]")),
                       ...
                       conditionalPanel("input.downloadData>0",br(),
                                        ...
                                        plotOutput("plotData",width = "900px", height = "600px")))),
  ...
  conditionalPanel(condition="input.Page==2",
    tabsetPanel(
       tabPanel("Find optimal DIA breakpoints", list(br(),
                ...
                plotOutput("ERBPlot",width = "900px", height = "600px")),value=2),
       tabPanel("Assess uncertainty in estimate", list(br(),
                 verbatimTextOutput("bootERB"),plotOutput("bootPlot",width="900px",height="600px")),value=3),
       tabPanel("Evaluate index for selected breakpoints", list(br(),
                verbatimTextOutput("ERBCalc2"),
                plotOutput("ERBPlot2",width = "900px", height = "600px")),value=4),
       id='panel'
    )),
...
#   progressInit(),
  conditionalPanel(condition="input.Page==3",
       tabsetPanel(
         tabPanel("Logistic Model",br(), list(p(strong("This function will take several minutes to run.")),
                  br(),verbatimTextOutput("logistic"),br(),br(),
                  plotOutput("logisticPlot",width = "900px", height = "600px"),br(),
                  plotOutput("plotLogisticDensity",width = "900px", height = "600px")),value=2),
         tabPanel("Spline Model",br(), list(p(strong("This function will take several minutes to run.")),
                   br(),
                   verbatimTextOutput("spline"),
                   plotOutput("splinePlot",width = "900px", height = "600px"),br(),
                   plotOutput("plotSplineDensity",width = "900px", height = "600px")),value=3),
         tabPanel("Compare Model Fits", list(br(),
                   verbatimTextOutput("compareFits"),
                   plotOutput("compareFitsPlot",width = "900px", height = "900px")),value=4),
         tabPanel("DIA Classification Probability", list(
                 br(),
                 p(strong("MIC breakpoint(s) shifted left 0.5 units to account for upwards rounding.")),
                 br(),                                
                 verbatimTextOutput("probDIA"),
                 plotOutput("probDIAPlot",width = "900px", height = "600px"),br(),
                 plotOutput("plotDist",width = "900px", height = "600px")),value=5),
         id='panel2'
       ))
  )
)
